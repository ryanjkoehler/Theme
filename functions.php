		...
		if ( ! empty( $data['blog_id'] ) && switch_to_blog( $data['blog_id'], true ) ) {
			...
			if ( current_user_can( 'publish_posts' ) ) {
				...
				switch ( $data['post_format'] ) {
					...
					case 'quote':
						$content = '<blockquote>' . $data['quote'] . '</blockquote>' . "\n" . '<cite>' . $data['source'] . '</cite>' . "\n" .  $data['caption'];
					break;
					
					case 'image':
						global $content_width;
						if ( ! empty( $data['url'] ) ) {
							$image_info = getimagesize( $data['url'] );
							...
							if ( in_array( $image_info['mime'], $mimes ) ) {
								...
								$html = '<img src="' . $data['url'] . '" width="' . $new_width . '" height="' . $new_height . '" />' . "\n\n";
								$content = $html . "\n\n" . $data['caption'];
							} else if ( wp_oembed_get( $data['url'] ) != '' ) {
								$content = $data['url'] . "\n\n" . $data['caption'];
							}
						}
...
function socd_qp_ajax_tax() {
	if ( ! empty( $_POST['tax_nonce'] ) && wp_verify_nonce( $_POST['tax_nonce'], 'socd_tax' ) ) {
		...
		// Switch to correct blog
		if ( ! empty( $_POST['blog_id'] ) && switch_to_blog( $_POST['blog_id'], true ) ) {
